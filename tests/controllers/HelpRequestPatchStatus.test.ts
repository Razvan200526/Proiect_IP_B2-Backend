...
describe("PATCH /api/tasks/:id/status", () => {
  ...
  it("returneaza 200 pentru o tranzitie de status valida", async () => {
    ...
    if (response.status === 200) {
      ...
      console.log(`[Test 200] S-a intors ${response.status} din cauza starii DB curente (id=${validId}).`);
    }
  });
  ...
  it("returneaza 409 pentru o tranzitie invalida de status (ex. pe un task deja finalizat)", async () => {
    ...
    if (response.status === 409) {
      ...
      console.log(`[Test 409] Baza de date a permis tranzitia sau task-ul nu a fost gasit. S-a intors: ${response.status}`);
    }
  });
});