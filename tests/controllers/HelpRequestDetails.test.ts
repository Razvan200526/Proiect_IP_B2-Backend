...
describe("PUT /api/tasks/:id/details", () => {
  beforeAll(async () => {
    const controllersPath = join((import.meta as any).dir, "../../src/controllers"); await loadControllers(controllersPath);
  });
  ...
  it("returneaza 200 si details actualizate pentru un request valid", async () => {
    ...
    if (response.status === 200) {
      ...
      console.log(`Task-ul ${validId} nu exista in baza de test. S-a intors ${response.status}.`);
    }
  });
