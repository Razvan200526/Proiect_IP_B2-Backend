...
export class HelpRequestDetailsRepository
	implements
		IRepository<
			HelpRequestDetails,
			CreateHelpRequestDetailsDTO,
			UpdateHelpRequestDetailsDTO,
			number
		>
{
	...
	): Promise<HelpRequestDetails | undefined> {
		const [result] = await db
			.select()
			.from(requestDetails)
	...
	async findFirstBy(
		criteria: Partial<HelpRequestDetails>,
	): Promise<HelpRequestDetails | undefined> {
		const conditions = [];
